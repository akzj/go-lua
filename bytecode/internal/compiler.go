...
func (fs *FuncState) compileFuncDef(funcDef astapi.FuncDef) (*Prototype, error) {
	...
	// DEBUG: dump generated bytecode for nested functions
	fmt.Printf("DEBUG compileFuncDef: numparams=%d maxstack=%d peakStack=%d\n",
		nestedProto.numparams&0x7F, nestedProto.maxstacksize, nestedFs.peakStack)
	for li, loc := range nestedFs.locals.vars {
		fmt.Printf("  local[%d]: name=%s reg=%d\n", li, loc.Name, loc.Reg)
	}
	for ui, uv := range nestedProto.upvalues {
		fmt.Printf("  upval[%d]: name=%s instack=%d idx=%d\n", ui, uv.Name, uv.Instack, uv.Idx)
	}
	for ci, inst := range nestedProto.code {
		fmt.Printf("  [%d] %08x\n", ci, inst)
	}
